      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          <div className="w-full sm:w-1/3">
            ...
            <select value={selectedClassType} onChange={(e) => setSelectedClassType(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm">
          ...
          <div className="w-full sm:w-2/3">
            ...
            <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm" />
          </div>
        </div>
              ...
              {currentTeachers.map((teacher) => (
                <div key={teacher._id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl">
                  ...
                  <div className="p-6 flex flex-col gap-2">
                    <p className="text-sm"><strong>CNIC:</strong> {teacher.cnic}</p>
                    <p className="text-sm"><strong>Phone:</strong> {teacher.phoneNumber}</p>
                    <p className="text-sm"><strong>Address:</strong> {teacher.address}</p>
                    <p className="text-sm"><strong>Class Type:</strong> {teacher.classType}</p>
                  </div>
                  <div className="flex justify-between items-center p-4 bg-gray-50">
                    <a href={`http://localhost:5000/${teacher.cv}`} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:text-blue-800">View CV</a>
